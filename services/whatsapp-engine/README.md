...
- The background engine uses the same protected real-send logic when real sending is enabled.
...
????? ?????:
- `pm2 status`
- `pm2 logs yemenstack-whatsapp-engine`

??????? ???????:
- ??? ????? ??? VPS ?? ???? QR ??? ?? ??? ???? ???? ??????.
- ??? ??? QR ?? ?????? ???? ?????? ???????? ??? `WhatsApp client is ready`.
- ??????? ??????? ???? ?????? ?????????? ??? ???? ??? ?? `ENABLE_REAL_WHATSAPP_SEND=true`.
- ????? ???????? ????? ??? VPS ???? ??? `WHATSAPP_TEST_RECIPIENT` ??? ??? ???? ???.
- ??? ?????? ?? ????? ??????? ???? ??? `WHATSAPP_TEST_RECIPIENT` ?????? ?????? ???????? ??? `WHATSAPP_SEND_LIMIT`.
- ??? `ecosystem.config.cjs` ?? ????? ?? Token ????? ?? ?????.